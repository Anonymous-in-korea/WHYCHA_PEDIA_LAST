	...
	<head>
		...
		<script>
			$(function() {
				$("#btn1").click(function() {
					let is_regi = 2;
					$("#noticeModify").attr("action", "/admin/1_notice/notice_modify?is_regi=" + is_regi);
		        ...
		        <main>
		            <div class="container-fluid px-4">
		                ...
		                <div class="card mb-4">
		                    ...
		                    <form action="" method="post" name="notice_modify" id="noticeModify">
		                    	...
		                    	<c:if test="${ result == 1 }">
			                        <input type="text" class="fo" name="announcement_title" id="announcement_title" placeholder="제목을 입력하세요." onfocus="this.placeholder=''"
			                        	onblur="this.placeholder='제목을 입력하세요.'" value="${ announcementVo.announcement_title }">
		                    	</c:if>
		                    	<c:if test="${ result == 0 }">
			                        <input type="text" class="fo" name="announcement_title" id="announcement_title" placeholder="제목을 입력하세요." onfocus="this.placeholder=''"
		                        ...
		                        <div class="centerdiv">
		                            ...
		                            <c:if test="${ result == 1 }">
			                            <p>
			                            	<textarea name="announcement_content" id="announcement_content" placeholder="공지할 내용을 입력해 주세요. (3000자 이내)" onfocus="this.placeholder=''"
			                            		onblur="this.placeholder='공지할 내용을 입력해 주세요. (3000자 이내)'"><c:out value="${ announcementVo.announcement_content }" /></textarea>
			                            </p>
		                            </c:if>
		                            <c:if test="${ result == 0 }">
			                            <p>
		                            ...
		                            <c:if test="${ announcementVo.announcement_url != '0' }">
			                            <p>
			                            	<input type="text" name="announcementUrl" id="announcementUrl" placeholder="입력할 이미지의 URL을 입력하세요."
			                            		onfocus="this.placeholder=''" onblur="this.placeholder='입력할 이미지의 URL을 입력하세요.'" value="${ announcementVo.announcement_url }">
			                            </p>
		                            </c:if>
		                            <c:if test="${ announcementVo.announcement_url == '0' }">
			                            <p>
			                            	<input type="text" name="announcementUrl" id="announcementUrl" placeholder="입력할 이미지의 URL을 입력하세요." onfocus="this.placeholder=''"
		                    ...
		                    <div id="btn0">
		                        <button type="button" class="butn" id="btn1">수정저장하기</button>
		                        <c:if test="${ announcementVo.is_regi == 1 }">
		                        <button type="button" class="butn" id="btn2">바로등록하기</button>
		                        </c:if>
		                        <button type="button" class="butn" id="btn3" onclick="location.href='/admin/1_notice/notice_list'">수정취소하기</button>
		                    </div>
		                </div>
		            </div>
		        </main>
