								...
								<div class="datatable-wrapper datatable-loading no-footer sortable searchable fixed-columns">
		                    		<div class="datatable-top">
		                    			<div class="datatable-dropdown">
		                    				<form action="/admin/5_character/actor_manage" method="get" name="datatableOption">
			                    				<label>
	                    				...
	                    				<button id="btn1" style="width:200px; height:40px; margin-left:15px; float:right;" onclick="location.href='/admin/5_character/character_input'">배우정보 등록하기</button>
		                    			<form action="/admin/5_character/actor_manage" method="get" name="actorSearch">
		                    		...
		                    		<div>
			                    		<div class="numbering">
											...
											<c:if test="${ now_page > 1 }">
												<a href="/admin/5_character/actor_manage?page=1&searchWord=${ searchWord }&datatableSelector=${ datatableSelector }"><span>&#171;</span></a>
											</c:if>
											<c:if test="${ now_page == 1 }"><span>&#171;</span></c:if>
											
											<!-- 이전페이지로 이동하기 -->
											<c:if test="${ now_page > 1 }">
												<a href="/admin/5_character/actor_manage?page=${ now_page - 1 }&searchWord=${ searchWord }&datatableSelector=${ datatableSelector }"><span>&#60;</span></a>
											</c:if>
											<c:if test="${ now_page == 1 }"><span>&#60;</span></c:if>
											...
											<c:forEach begin="${ startPage }" end="${ endPage }" step="1" var="num">
												...
												<c:if test="${ now_page != num }">
													<a href="/admin/5_character/actor_manage?page=${ num }&searchWord=${ searchWord }&datatableSelector=${ datatableSelector }"><span>${ num }</span></a>
												</c:if>
											</c:forEach>
											...
											<!-- 마지막페이지로 이동하기 -->
											<c:if test="${ now_page < maxPage }">
												<a href="/admin/5_character/actor_manage?page=${ maxPage }&searchWord=${ searchWord }&datatableSelector=${ datatableSelector }"><span>&#187;</span></a>
											</c:if>
											<c:if test="${ now_page == maxPage }"><span>&#187;</span></c:if>
										</div>
		                    		</div>
