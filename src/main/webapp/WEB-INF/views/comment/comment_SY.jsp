...
<!-- 		<script src="../js/comment_SY.js"></script> -->
		<script>
		$(function(){
		                                 ...
		                                 <!--ul 코멘트 리스트2 시작 -->
		                                    <div class="css-bawlbm" >
		                                        ...
		                                        <div class="css-4tkoly">
		                                        	<a class="css-1f9m1s4-StylelessLocalLink eovgsd01" href="comment_reply_GC?comment_id=${commentVolist[i].id}&movie_id=${commentVolist[i].movie_id}">
		                                        ...
		                                        <div class="css-hy68ty" >
													...
													<c:forEach var="j" begin="0" end="${likeList.size()-1}">
													  <c:if test="${commentVolist[i].id == likeList[j].comment_id}">
													    <c:if test="${likeList[j].user_id == sessionId}">
													      <c:set var="likedByUser" value="true" />
													    </c:if>
													  </c:if>
													</c:forEach>
													<c:if test="${likedByUser == true}">
													  <button type="button" class="css-jj4q3s-StylelessButton-UserActionButton commentLike" id="${commentVolist[i].id}">좋아요</button>
													</c:if>
													<c:if test="${likedByUser == false}">
													  <button type="button" class="css-1h18l7j-StylelessButton commentLike" id="${commentVolist[i].id}">좋아요</button>
													</c:if>
		                                        </div>
		                                    </div>
		                           		</c:forEach>
